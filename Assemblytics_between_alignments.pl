...
sub findoverlap {
	...
	my @sorted_indexes_indexes = sort { $sorted_indexes[$a] <=> $sorted_indexes[$b] } 0..$#sorted_indexes;
	
	
	return @sorted_indexes_indexes;
	}
...
while (<COORDS>)
{
  ...
  if ($a->{"qrc"} == 0) # if the query has more than 1 alignment
  {
  $neg++;}
            ...
            if ($longrange_filter ne "exclude-longrange" || ($typeguess ne "Interchromosomal" && $typeguess ne "Longrange")) {
              if ($longrange_filter ne "longrange-only" || ($typeguess eq "Interchromosomal" || $typeguess eq "Longrange")) {
                if ($output_file eq "bedpe") {
                  ...
                  print "$chromi\t$ref_start\t$ref_stop\tAssemblytics_b_$sv_id_counter\t$abs_event_size\t$strandj\t$typeguess\t$rdist\t$qdist\t$qpos\tbetween_alignments\n";  
                }
              }
            }
          }
